...
(function () {
    ...
    function PropertyFilter (property, operator, value) {
        this.sql = function (prefix, values) {
            if (operator === '=' && value === null) {
                ...
                values.push(asyncorm.entityValToDbVal(value));
    ...
    dbQueryCollection = function (entityName) {
        ...
        var meta = asyncorm.getMeta(entityName);

        that.list = function (tx, callback) {

            function selectAll (meta, prefix) {
                var selectFields = [ "`" + meta.name + "`.id AS " + prefix
                        + "id" ];
                for ( var p in meta.fields) {
                    if (meta.fields.hasOwnProperty(p)) {
                        selectFields.push("`" + meta.name + "`.`" + p
                                + "` AS `" + prefix + p + "`");
                    }
                }
            ...
            var selectFields = selectAll(meta, mainPrefix);
            ...
            for ( var i = 0; i < that._prefetchFields.length; i++) {
                ...
                console.log(thatMeta.name)
                selectFields = selectFields.concat(selectAll(thatMeta,
                        prefetchField + "_"));
                joinSql += "LEFT JOIN `" + thatMeta.name + "` ON `" + prefetchField
                        + "`.`id` = `" + mainPrefix + prefetchField + "` ";
                ...
                tx.executeSql(sql, args,
                        function (rows) {
                            ...
                            for ( var i = 0; i < rows.length; i++) {
                                var r = rows[i];
                                var e = asyncorm.rowToEntity(entityName, r,
                                        mainPrefix);
                                results.push(e);
                                asyncorm.add(e);
                            }
    ...
    asyncorm.dbQueryCollection = dbQueryCollection;
}());