...
function config(persistence, dialect) {
  ...
  function save(obj, tx, callback) {
    ...
    executeQueriesSeq(tx, additionalQueries, function() {
        ...
        if (obj._new) {
          ...
          qs.push(tm.outIdVar('?'));
          sql = "INSERT INTO `" + obj._type + "` (" + properties.join(", ") + ") VALUES (" + qs.join(', ') + ")";
          obj._new = false;
        } else {
	...
	catch( exc )
	{
		...
		{
			console.warn( exc.message, sql, values );
		}
  ...
  persistence.DbQueryCollection.prototype.list = function (tx, callback) {
	...
	catch( exc )
	{
		if( window.console && typeof( console.warn ) === 'function' )
		{
			console.warn( exc.message, sql, args );
		}
		callback([]);
	}
      });
  ...
  persistence.DbQueryCollection.prototype.destroyAll = function (tx, callback) {
	...
	catch( exc )
	{
		if( window.console && typeof( console.warn ) === 'function' )
		{
			console.warn( exc.message, selectSql, args );
		}
		if( typeof( callback ) === 'function' )
		{
			callback();
