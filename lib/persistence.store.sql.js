...
function config(persistence, dialect) {
  ...
  persistence.DbQueryCollection.prototype.list = function (tx, callback) {
    ...
    this._prefetchFields.forEach(function(prefetchField) {
      if(!(prefetchField in meta.hasOne)) {
        throw(new ReferenceError( "'" + meta.name + "' does not 'hasOne' field called '" + prefetchField + "'."));
      }
